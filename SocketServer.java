...
public class SocketServer {
    ...
    private static final int PORT = 8080; // 서버 포트
    public static int clientCount = 0; // 접속한 클라이언트 수
    private static final LocalDateTime startTime = LocalDateTime.now(); // 서버 시작 시간
    private static final String LOG_DIR = "logs"; // 로그 폴더명
    private static final String LOG_FILE = getLogFileName(); // 로그 파일명
    ...
    // 서버 실행 시간 계산 함수
    private static String calculateUptime(LocalDateTime start, LocalDateTime now) {
    ...
    // 클라이언트가 종료될 때 호출할 함수
    public static synchronized void clientDisconnected(InetAddress clientAddress) {
